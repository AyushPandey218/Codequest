...
const diffColor = { easy: 'text-green-400', medium: 'text-yellow-400', hard: 'text-red-400', 'Easy': 'text-green-400', 'Medium': 'text-yellow-400', 'Hard': 'text-red-400' }

const AdminAnalytics = () => {
    const { dailyActiveUsers, questCompletions, newSignups, topQuests, levelDistribution, isLoading } = useAdminAnalytics()
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-white/5 text-slate-500 text-xs uppercase tracking-wider">
                                ...
                                <th className="text-left px-5 py-3">Quest ID</th>
                            ...
                            {topQuests.map((q, i) => (
                                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-5 py-3">
                                        <span className={`font-medium text-xs ${diffColor[q.difficulty] || 'text-slate-400'} capitalize`}>{q.difficulty}</span>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="lg:col-span-2 bg-[#12122a] border border-white/5 rounded-2xl p-5 space-y-5">
                    ...
                    <div className="pt-4 border-t border-white/5 space-y-3">
                        ...
                        {isLoading ? (
                            <div className="animate-pulse flex space-x-4"><div className="flex-1 space-y-6 py-1"><div className="space-y-3"><div className="grid grid-cols-3 gap-4"><div className="h-2 bg-white/20 rounded col-span-2"></div><div className="h-2 bg-white/20 rounded col-span-1"></div></div><div className="h-2 bg-white/20 rounded"></div></div></div></div>
                        ) : (
                            <>
                                <BarRow label="Level 1–10" pct={levelDistribution.l1_10 || 0} color="bg-slate-500" />
                                <BarRow label="Level 11–30" pct={levelDistribution.l11_30 || 0} color="bg-blue-500" />
                                <BarRow label="Level 31–50" pct={levelDistribution.l31_50 || 0} color="bg-purple-500" />
                                <BarRow label="Level 51+" pct={levelDistribution.l51_plus || 0} color="bg-yellow-400" />
                            </>
                        )}
                    </div>
                </div>
